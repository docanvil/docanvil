...
pub fn run(project_root: &Path, overwrite: bool) -> Result<()> {
    ...
    let light = if color_mode != "dark" {
        let label = if color_mode == "both" {
            "Light mode primary"
        } else {
            "Primary color"
        };
        Some(prompt_colors(label, "#6366f1", "#f97316"))
    } else {
        None
    };

    // Prompt for dark colors (if dark or both)
    let dark = if color_mode != "light" {
        let label = if color_mode == "both" {
            "Dark mode primary"
        } else {
            "Primary color"
        };
        Some(prompt_colors(label, "#818cf8", "#f97316"))
    } else {
        None
    };

    generate_theme(project_root, &config, color_mode, light, dark, overwrite)
}

/// Prompt for primary + secondary hex colors with the given label prefix.
fn prompt_colors(label: &str, default_primary: &str, default_secondary: &str) -> ThemeColors {
...
fn generate_theme(
    ...
    let css = match color_mode {
        ...
        _ => {
            // "both"
            let lc = light.as_ref().unwrap();
            let dc = dark.as_ref().unwrap();
            format!(
                "/* DocAnvil custom theme — generated by `docanvil theme`\n\
                 *\n\
